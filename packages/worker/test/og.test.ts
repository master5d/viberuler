...
describe('GET /og/:login.png', () => {
  ...
  it('renders a PNG for the path-versioned URL (version segment is ignored)', async () => {
    const res = await exports.default.fetch('https://viberuler.dev/og/master5d/20260709171746.png');
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toContain('image/png');
  });
