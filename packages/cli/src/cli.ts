...
export async function main(
  ...
  if (command === 'audit') {
    if (values.compare !== undefined) {
      ...
      const actxA: ScanContext = { home, agentHomes, scanDirs: [], since: range.windowA.since, until: range.windowA.until, authorEmail: undefined, env: process.env };
      const actxB: ScanContext = { home, agentHomes, scanDirs: [], since: range.windowB.since, until: range.windowB.until, authorEmail: undefined, env: process.env };
      const report = await runAuditCompare(actxA, actxB, range.windowA, range.windowB, gapMs);
